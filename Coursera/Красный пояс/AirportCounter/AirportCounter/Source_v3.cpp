...
public:
	...
	{
		return items[static_cast<size_t>(airport)];
	}

	// добавить данный элемент
	void Insert(TAirport airport)
	{
		items[static_cast<size_t>(airport)]++;
	}

	// удалить одно вхождение данного элемента
	void EraseOne(TAirport airport)
	{
		size_t& flights = items[static_cast<size_t>(airport)];
		if (flights > 0)
		{
			flights--;
		}
	...
	{
		items[static_cast<size_t>(airport)] = 0;
	}
	...
	Items GetItems() const
	{
		...
		{
			result[static_cast<TAirport>(i)] = items[i];
		}
		return result;
	}
...
private:
	...
	size_t size_ = static_cast<size_t>(TAirport::Last_);
};
