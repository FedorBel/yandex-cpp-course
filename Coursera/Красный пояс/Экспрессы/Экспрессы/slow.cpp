...
public:
	void AddRoute(int start, int finish) {
		reachable_lists_[start].insert(finish); // logQ
		reachable_lists_[finish].insert(start); //logQ
	}
	int FindNearestFinish(int start, int finish) const {
		...
		const auto finish_pos = reachable_stations.lower_bound(finish); // log Q
...
public:
	void AddRoute(int start, int finish) {
		reachable_lists_[start].push_back(finish); // Q
		reachable_lists_[finish].push_back(start); // Q
	}
	int FindNearestFinish(int start, int finish) const {
		int result = abs(start - finish);
		if (reachable_lists_.count(start) < 1) {
