...
public:
    ...
    {
        size_t hashed_key = hasher(key) % data.size();
        auto &bucket = data[hashed_key];
        return {key, bucket};
    }
    ...
    ReadAccess At(const K &key) const
    {
        size_t hashed_key = hasher(key) % data.size();
        auto &bucket = data[hashed_key];
        if (bucket.second.find(key) == bucket.second.end())
        {
    ...
    bool Has(const K &key) const
    {
        size_t hashed_key = hasher(key) % data.size();
        auto &bucket = data[hashed_key];
        return (bucket.second.find(key) != bucket.second.end());
    }
